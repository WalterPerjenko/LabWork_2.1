...
public class StorageManager {
    ...
    public List<placement> listAllPlacements() {
...
// container
    ...
    public void removeSlot(long slotId) {
        ...
        if (slot == null) {
            throw new IllegalArgumentException("Ошибка: слот не найден");
        }
        if (slot.isOccupied()) {
            throw new IllegalArgumentException("Ошибка: нельзя удалить занятый слот");
        }
        slots.remove(slot);
    }
    ...
    public void loadFromDTO(StorageDataDTO data) {
        ...
        for (ContainerDTO dto : data.getContainers()) {
            container c = new container(
                    dto.getId(), dto.getName(), ContainerType.valueOf(dto.getType()),
        ...
        for (SlotDTO dto : data.getSlots()) {
            slot s = new slot(dto.getId(), dto.getContainerId(), dto.getCode());
            s.setOccupied(dto.isOccupied());
            slots.add(s);
        }


        for (PlacementDTO dto : data.getPlacements()) {
            placement p = new placement(
                    dto.getId(), dto.getSampleId(), dto.getContainerId(),
                    dto.getSlotId(), dto.getOwnerUsername()
            );
            placements.add(p);
        }
    ...
    public boolean validateData(StorageDataDTO data) {
        if (data == null) return false;
        if (data.getContainers() == null || data.getSlots() == null || data.getPlacements() == null) {
            return false;
        }
        // Проверка уникальности ID
        Set<Long> containerIds = new HashSet<>();
        for (ContainerDTO c : data.getContainers()) {
            if (!containerIds.add(c.getId())) return false;
        }
        Set<Long> slotIds = new HashSet<>();
        for (SlotDTO s : data.getSlots()) {
            if (!slotIds.add(s.getId())) return false;
            if (!containerIds.contains(s.getContainerId())) return false;
        }
        Set<Long> placementIds = new HashSet<>();
        Set<Long> usedSlotIds = new HashSet<>();
        for (PlacementDTO p : data.getPlacements()) {
            if (!placementIds.add(p.getId())) return false;
            if (!slotIds.contains(p.getSlotId())) return false;
            if (!usedSlotIds.add(p.getSlotId())) return false; // слот занят только 1 образцом
        }
        return true;
    }
}